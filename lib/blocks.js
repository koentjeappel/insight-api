...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix pacglobal to give back null instead of null hash
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.dashd.getRawBlock(hash, function(err, blockBuffer) {
      ...
      var br = new pacglobal.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in pacglobal-node.
      var header = pacglobal.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [pacglobal.Transaction().fromBufferReader(br)];
